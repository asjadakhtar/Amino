    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      if ( $projects_query->have_posts() ):
        while ( $projects_query->have_posts() ): $projects_query->the_post();
          ...
          // Get project name - first try ACF field, then fallback to post title
          $name = get_field('project_name');
          if (!$name) {
              $name = get_the_title();
          }
          
          // Get project tags - first try ACF field, then fallback to taxonomy
          ...
          // Get featured image (WordPress default thumbnail)
          $featured_image = get_the_post_thumbnail_url(get_the_ID(), 'full');
          
          // Fallback to hero image if featured image not set
          if (!$featured_image) {
              $hero_image = get_field('project_hero_image');
              $featured_image = $hero_image ? $hero_image['url'] : '';
          }
      ...
      <div class="relative group w-full cursor-pointer mx-auto overflow-hidden rounded-xl">
        <a href="<?php echo esc_url($permalink); ?>" rel="noopener">
